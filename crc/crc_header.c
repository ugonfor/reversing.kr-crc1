...
unsigned __int64 crc_cacl(unsigned __int64 before_value,unsigned char text){

    return crc_table[(before_value ^ text) & 0xff] ^ (before_value >> 8);
}

unsigned __int64 crc_rev(unsigned __int64 after_value, unsigned char text){
    
    unsigned char top_byte = (unsigned char) (after_value >> 56);
    int index = 0;

    for (int i = 0; i < 256; i++)
    {
        if((unsigned char)(crc_table[i] >> 56) == top_byte){
            index = i;
            break;
        }
    }

    unsigned __int64 before_value = after_value ^ crc_table[index];
    before_value = before_value << 8;

    before_value = before_value | (unsigned char) index ^ text; 
...
{
    unsigned __int64 A = *(unsigned __int64*)a;
    unsigned __int64 B = *(unsigned __int64*)b;
    if (A > B) return 1;
